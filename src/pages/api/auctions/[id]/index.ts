...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    ...
    case 'DELETE':
      return deleteAuctionById();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getAuctionById() {
    const data = await prisma.auction.findFirst(convertQueryToPrismaUtil(req.query, 'auction'));
    return res.status(200).json(data);
  }

  async function updateAuctionById() {
  ...
  async function deleteAuctionById() {
